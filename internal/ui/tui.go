...
func NewTUI(submitFn func(SubmitRequest), useAltScreen bool, plainTTY bool, codexAvailable, claudeAvailable bool) *TUI {
	if plainTTY {
		EnablePlainTTY()
	}
	ready := make(chan struct{})
	m := NewTUIModel(codexAvailable, claudeAvailable)
	m.SubmitFn = submitFn
