...
datasets, CI, and improved token estimation. Phase 2 builds the capabilities
that differentiate v100 from commodity agent wrappers.

---

## 2a. Solver Abstractions

**Problem.** `Loop.Step()` is a flat ReAct cycle: user → model → tools → model →
… → final text. There is no branching, backtracking, plan generation, or
reflection. Every research harness competitor (Inspect, METR, DSPy) supports
at least plan-then-execute.

**Design.**

```
internal/core/solver.go      — interface + registry
internal/core/solver_react.go    — extract current loop into Solver impl
internal/core/solver_plan.go     — plan-then-execute
internal/core/solver_reflect.go  — reflexion (run → evaluate → reflect → retry)
internal/core/checkpoint.go      — message + workspace snapshot/restore
```

### Interface

```go
// Solver drives a task from prompt to completion using a Loop.
type Solver interface {
    Name() string
    Solve(ctx context.Context, loop *Loop, task string) (SolveResult, error)
}
...
type SolveResult struct {
    ...
    Steps      int
    Checkpoints int
    Retries    int     // for reflexion
}
```

### Implementations

**react** — Extract the current `Loop.Step()` inner loop as-is. This is the
default and preserves backward compatibility. `Loop.Step()` becomes a thin
wrapper that delegates to the active solver.

**plan_execute** — Two-phase solver:
1. *Plan phase*: Call model with tools=nil, system prompt asks for numbered
   step plan. Parse the plan into `[]PlanStep{description, done}`.
2. *Execute phase*: For each plan step, call the model with tools enabled and
   the current step as the user message. After each step, update the plan
   (mark done, optionally re-plan if a step fails).
3. *Re-plan*: If a step fails or the model requests it, go back to plan phase
   with the updated context. Cap re-plans at 3.

Plan is stored in messages as a system message so the model always sees current
state. Trace gets a new `solver.plan` event type with the plan text.

**reflexion** — Multi-attempt solver:
1. Run the task using the react solver.
2. Score the result using a Scorer (from Phase 1 eval package).
3. If score != pass, generate a *reflection* (model call analyzing what went
   wrong), prepend it to messages as a system message, and retry.
4. Cap retries at N (default 3). Return best result by score.

Requires: a Scorer and expected value. If no scorer is configured, reflexion
degrades to single-attempt react.

Trace events: `solver.attempt` (attempt number, score), `solver.reflect`
(reflection text).

### Checkpointing

Solvers that branch or retry need to snapshot and restore state.
...
type Checkpoint struct {
    ...
    StepCount int
    // Workspace snapshot is optional — Phase 2 skips filesystem
    // snapshots; Phase 3 sandbox adds them.
}
...
For Phase 2, checkpointing only captures message history and step count.
Workspace filesystem snapshots come with Phase 3 sandbox work.

### Config integration

```toml
[defaults]
solver = "react"          # default solver

[defaults.solver_options]
max_replans = 3           # plan_execute
max_retries = 3           # reflexion
reflexion_scorer = "model_graded"
```

CLI: `v100 run --solver plan_execute "implement feature X"`

Bench: add `solver` field to `BenchVariant` so experiments can compare solvers.

### Test plan

- Unit test: react solver produces same trace as current Loop.Step()
- Unit test: plan_execute generates plan, executes steps, re-plans on failure
  (mock provider returns canned plan then canned execution)
- Unit test: reflexion retries when scorer returns fail, stops when pass
- Unit test: checkpoint save/restore preserves message history
- Integration: run same prompt with react vs plan_execute, compare trace stats
